...
import { generateWhatsAppUrl } from '../config.js';

export function createWhatsAppButton(messageKey = 'general', tooltipText = 'Consultá por WhatsApp') {
  const button = document.createElement('a');
  button.href = generateWhatsAppUrl(messageKey);
...
/**
 * Inicializa el botón de WhatsApp en la página
 */
export function initWhatsAppButton(messageKey = 'general', tooltipText = 'Consultá por WhatsApp') {
  const button = createWhatsAppButton(messageKey, tooltipText);
...
/**
 * Configura todos los enlaces de WhatsApp en la página
 */
export function setupWhatsAppLinks() {
  ...
  whatsappLinks.forEach(link => {
    ...
    link.href = generateWhatsAppUrl(messageKey, customMessage);
    link.target = '_blank';
    link.rel = 'noopener noreferrer';
    
    link.addEventListener('click', () => {
      trackEvent('WhatsApp Click', {
        source: link.dataset.trackingSource || 'link',
        message: messageKey,
      });
    });
  });
